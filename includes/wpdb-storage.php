...
class WpdbStorage implements StorageInterface {
    private $wpdb;
    ...
    public function __construct( $wpdb_instance = null ) {
        ...
        $this->table_prefix = $this->wpdb->prefix . 'a8c_';
        $this->init_database();
    }
    ...
    private function create_tables() {
        ...
        // Teams table
        $sql = "CREATE TABLE {$this->table_prefix}teams (
        ...
        // People table
        $sql = "CREATE TABLE {$this->table_prefix}people (
        ...
        // Events table
        $sql = "CREATE TABLE {$this->table_prefix}events (
            id bigint(20) unsigned NOT NULL AUTO_INCREMENT,
            team_slug varchar(100) NOT NULL,
        ...
        // HR Feedback table
        $sql = "CREATE TABLE {$this->table_prefix}hr_feedback (
            id bigint(20) unsigned NOT NULL AUTO_INCREMENT,
            username varchar(100) NOT NULL,
        ...
        // Team links table
        $sql = "CREATE TABLE {$this->table_prefix}team_links (
            id bigint(20) unsigned NOT NULL AUTO_INCREMENT,
            team_slug varchar(100) NOT NULL,
        ...
        // People links table
        $sql = "CREATE TABLE {$this->table_prefix}people_links (
            id bigint(20) unsigned NOT NULL AUTO_INCREMENT,
        ...
        // Event links table
        $sql = "CREATE TABLE {$this->table_prefix}event_links (
            id bigint(20) unsigned NOT NULL AUTO_INCREMENT,
    ...
    private function run_migrations() {
        ...
        $charset_collate = $this->wpdb->get_charset_collate();
        $sql = "CREATE TABLE {$this->table_prefix}migrations (
            id bigint(20) unsigned NOT NULL AUTO_INCREMENT,
        ...
        $this->run_migration( 'add_linear_column_to_people', function() {
            ...
            $columns = $this->wpdb->get_results( "DESCRIBE {$this->table_prefix}people" );
            ...
            if ( ! $has_linear_column ) {
                $this->wpdb->query( "ALTER TABLE {$this->table_prefix}people ADD COLUMN `linear` varchar(100) DEFAULT ''" );
            }
        } );
    }
    ...
    private function run_migration( $migration_name, $migration_callback ) {
        $migration_exists = $this->wpdb->get_var( $this->wpdb->prepare(
            "SELECT COUNT(*) FROM {$this->table_prefix}migrations WHERE migration_name = %s",
            $migration_name
        ) );
    ...
    public function get_team_config( $team_slug ) {
        $team = $this->wpdb->get_row( $this->wpdb->prepare(
            "SELECT * FROM {$this->table_prefix}teams WHERE slug = %s",
            $team_slug
        ), ARRAY_A );
    ...
    private function get_people_by_category( $team_slug, $category ) {
        $results = $this->wpdb->get_results( $this->wpdb->prepare(
            "SELECT * FROM {$this->table_prefix}people WHERE team_slug = %s AND category = %s ORDER BY name",
            $team_slug, $category
        ), ARRAY_A );
    ...
    private function get_team_events( $team_slug ) {
        $results = $this->wpdb->get_results( $this->wpdb->prepare(
            "SELECT * FROM {$this->table_prefix}events WHERE team_slug = %s ORDER BY start_date",
            $team_slug
        ), ARRAY_A );
    ...
    public function save_team_config( $team_slug, $config ) {
        ...
        try {
            ...
            $existing_team = $this->wpdb->get_var( $this->wpdb->prepare(
                "SELECT COUNT(*) FROM {$this->table_prefix}teams WHERE slug = %s",
                $team_slug
            ) );
    ...
    public function get_available_teams() {
        return $this->wpdb->get_col( "SELECT slug FROM {$this->table_prefix}teams ORDER BY slug" );
    }
    ...
    public function get_team_name( $team_slug ) {
        return $this->wpdb->get_var( $this->wpdb->prepare(
            "SELECT team_name FROM {$this->table_prefix}teams WHERE slug = %s",
            $team_slug
        ) );
    }
    ...
    public function get_team_type( $team_slug ) {
        $type = $this->wpdb->get_var( $this->wpdb->prepare(
            "SELECT type FROM {$this->table_prefix}teams WHERE slug = %s",
            $team_slug
        ) );
        return $type ?: 'team';
    }
    ...
    public function get_default_team() {
        $slug = $this->wpdb->get_var( "SELECT slug FROM {$this->table_prefix}teams WHERE is_default = 1 LIMIT 1" );

        if ( ! $slug ) {
            // If no default team, return first available team
            $slug = $this->wpdb->get_var( "SELECT slug FROM {$this->table_prefix}teams ORDER BY slug LIMIT 1" );
        }
    ...
    public function get_hr_feedback( $username, $month = null ) {
        if ( $month ) {
            return $this->wpdb->get_row( $this->wpdb->prepare(
                "SELECT * FROM {$this->table_prefix}hr_feedback WHERE username = %s AND month = %s",
                $username, $month
            ), ARRAY_A );
        } else {
            $results = $this->wpdb->get_results( $this->wpdb->prepare(
                "SELECT * FROM {$this->table_prefix}hr_feedback WHERE username = %s ORDER BY month DESC",
                $username
            ), ARRAY_A );
    ...
    public function save_hr_feedback( $username, $month, $data ) {
        ...
        $existing_feedback = $this->wpdb->get_var( $this->wpdb->prepare(
            "SELECT COUNT(*) FROM {$this->table_prefix}hr_feedback WHERE username = %s AND month = %s",
            $username, $month
        ) );
    ...
    public function get_team_people_count( $team_slug ) {
        return (int) $this->wpdb->get_var( $this->wpdb->prepare(
            "SELECT COUNT(*) FROM {$this->table_prefix}people WHERE team_slug = %s",
            $team_slug
        ) );
    }
    ...
    public function get_team_people_names( $team_slug ) {
        return $this->wpdb->get_col( $this->wpdb->prepare(
            "SELECT name FROM {$this->table_prefix}people WHERE team_slug = %s ORDER BY name",
            $team_slug
        ) );
    }
    ...
    public function get_team_people_data( $team_slug ) {
        $results = $this->wpdb->get_results( $this->wpdb->prepare(
            "SELECT * FROM {$this->table_prefix}people WHERE team_slug = %s ORDER BY name",
            $team_slug
        ), ARRAY_A );
    ...
    private function get_team_links( $team_slug ) {
        $results = $this->wpdb->get_results( $this->wpdb->prepare(
            "SELECT link_name, link_url FROM {$this->table_prefix}team_links WHERE team_slug = %s ORDER BY link_name",
            $team_slug
        ), ARRAY_A );
    ...
    private function get_person_links( $person_id ) {
        $results = $this->wpdb->get_results( $this->wpdb->prepare(
            "SELECT link_name, link_url FROM {$this->table_prefix}people_links WHERE person_id = %d ORDER BY link_name",
            $person_id
        ), ARRAY_A );
    ...
    private function get_event_links( $event_id ) {
        $results = $this->wpdb->get_results( $this->wpdb->prepare(
            "SELECT link_name, link_url FROM {$this->table_prefix}event_links WHERE event_id = %d ORDER BY link_name",
            $event_id
        ), ARRAY_A );
